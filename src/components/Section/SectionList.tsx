...
import { PlusCircle, X, Settings, Search, Plus, Edit, Trash, Table, TableProperties, Maximize2, Minimize2, Unlink, Link, Clock, Layers } from "lucide-react";
...
import { CellSelection, MergedCell, MergedFields, Section, SectionListProps, SectionRowProps, SectionWithLevel, SelectionState } from "../../types/sections";
...
const sectionLevelColors = {
  0: { bg: "bg-gradient-to-r from-white to-blue-50/30", border: "border-l-[3px] border-l-blue-500", text: "text-gray-900", hover: "hover:bg-blue-50/40", indicator: "bg-gradient-to-r from-blue-500 to-blue-600" },
  1: { bg: "bg-gradient-to-r from-white to-indigo-50/20", border: "border-l-[3px] border-l-indigo-400", text: "text-gray-800", hover: "hover:bg-indigo-50/30", indicator: "bg-gradient-to-r from-indigo-400 to-indigo-500" },
  2: { bg: "bg-gradient-to-r from-white to-violet-50/10", border: "border-l-[3px] border-l-violet-400", text: "text-gray-700", hover: "hover:bg-violet-50/20", indicator: "bg-gradient-to-r from-violet-400 to-violet-500" },
};
...
const findAllSections = (sections: Section[]): SectionWithLevel[] => {
  ...
  const traverse = (section: Section, level = 0) => {
    ...
    section.subsections?.forEach((subsection) => traverse(subsection, level + 1));
  };
...
const getSectionValue = (section: SectionWithLevel, key: keyof MergedFields) => section[key as keyof Section];

const generateSectionColors = (level: number) => {
  const colorSchemes = {
    0: { bg: "from-blue-50 via-blue-50 to-white", border: "border-blue-200", hover: "hover:from-blue-200/80 hover:to-blue-50", text: "from-blue-700 to-blue-500", dot: "from-blue-500 to-blue-600", ring: "ring-blue-100", indicator: "M" },
    1: { bg: "from-indigo-50 via-indigo-50 to-white", border: "border-indigo-200", hover: "hover:from-indigo-100/80 hover:to-indigo-50", text: "from-indigo-700 to-indigo-500", dot: "from-indigo-500 to-indigo-600", ring: "ring-indigo-100", indicator: "S1" },
    2: { bg: "from-violet-50 via-violet-50 to-white", border: "border-violet-200", hover: "hover:from-violet-100/80 hover:to-violet-50", text: "from-violet-700 to-violet-500", dot: "from-violet-500 to-violet-600", ring: "ring-violet-100", indicator: "S2" },
    3: { bg: "from-purple-50 via-purple-50 to-white", border: "border-purple-200", hover: "hover:from-purple-100/80 hover:to-purple-50", text: "from-purple-700 to-purple-500", dot: "from-purple-500 to-purple-600", ring: "ring-purple-100", indicator: "S3" },
    4: { bg: "from-fuchsia-50 via-fuchsia-50 to-white", border: "border-fuchsia-200", hover: "hover:from-fuchsia-100/80 hover:to-fuchsia-50", text: "from-fuchsia-700 to-fuchsia-500", dot: "from-fuchsia-500 to-fuchsia-600", ring: "ring-fuchsia-100", indicator: "S4" },
  };
...
const getLevelIndicator = (level: number) => {
  ...
  return (
    <div className="flex items-center h-full" style={{ paddingLeft: `${indentWidth}px` }}>
      <div className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-gradient-to-r ${colorSet.bg} border ${colorSet.border} group ${colorSet.hover} transition-all duration-200`}>
        <div className="relative">
          <div className={`w-2 h-2 rounded-full bg-gradient-to-r ${colorSet.dot} ring-2 ${colorSet.ring}`} />
          <div className="absolute -inset-1 bg-current/20 rounded-full animate-pulse group-hover:animate-none opacity-50" />
        </div>
        <span className={`text-xs font-medium tracking-wide bg-gradient-to-r ${colorSet.text} bg-clip-text text-transparent`}>
...
const SectionRow = React.memo(({
  ...
  const getLevelColor = useCallback((level: number, index: number) => {
    if (level === 0) {
      // Update CSS variables for gradient
      const style = document.documentElement.style;
      style.setProperty('--gradient-start', tableStyles.mainSectionGradientStart);
      style.setProperty('--gradient-end', tableStyles.mainSectionGradientEnd);

      const gradientStyle = tableStyles.mainSectionGradientStart && tableStyles.mainSectionGradientEnd
        ? 'bg-gradient-to-r from-[var(--gradient-start)] to-[var(--gradient-end)]'
        : 'bg-gradient-to-r from-blue-400/50 to-white';

      const alternateStyle = tableStyles.alternateRowColors && index % 2 === 1
  ...
  const getMergedCellInfo = useCallback((sectionId: string, columnType: keyof MergedFields) => {
    return selection?.mergedCellsHistory.find((cell) => cell.sectionIds.includes(sectionId) && cell.columnType === columnType);
  }, [selection]);

  const isColumnSelected = useCallback((columnType: keyof MergedFields) => {
    const cellId = `${section.id}-${columnType}`;
    return selection?.selectedCells.some((cell) => cell.cellId === cellId);
  }, [section.id, selection]);

  const getColumnClassName = useCallback((columnType: keyof MergedFields) => {
    ...
    const textColor = level === 0 ? sectionLevelColors[0].text : `text-[${tableStyles.cellTextColor}]`;
    const bgColor = level === 0 ? 'transparent' : `bg-[${tableStyles.tableBackgroundColor}]`;
  ...
  }, [getMergedCellInfo, isColumnSelected, level, section.id, selection, tableStyles]);

  const handleColumnClick = useCallback((columnType: keyof MergedFields) => {
    if (!onSelect || !selection?.isSelecting) return;
    const cellId = `${section.id}-${columnType}`;
    onSelect(cellId, section.id, columnType);
  }, [onSelect, section.id, selection]);

  const renderTimeSlot = useCallback((timeSlot: TimeSlot, level: number, sectionId: string) => {
    const baseIndent = 20;
    ...
    return (
      ...
      >
        {level > 0 ? (
          <div className="absolute flex items-center h-full" style={{ left: `${indentWidth}px` }}>
            <div className="absolute h-full w-px bg-indigo-200/60" style={{ left: "-1px", top: "-50%" }} />
            <div className="h-px w-3 bg-indigo-200/60" />
            <div className={`w-1.5 h-1.5 rounded-sm border border-indigo-300/60 ${level === 1 ? "bg-indigo-50" : "bg-white"}`} />
          </div>
        ) : (
          <div className="absolute left-0 top-0 bottom-0 w-1 bg-blue-500/20" />
        )}
        <span
          className={`whitespace-nowrap flex items-center gap-2 ${level === 0 ? "text-gray-700 font-medium" : ""}`}
          style={{ 
  ...
  const renderColumnContent = useCallback((columnType: keyof MergedFields) => {
    const mergedCell = getMergedCellInfo(section.id, columnType);
    const isSelected = isColumnSelected(columnType);
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between h-full">
          <div className={columnType === "timeSlot" ? "" : "px-2"}>{content}</div>
          {mergedCell && isActiveSection && (
            <span className="text-xs text-gray-600">{mergedCell.mergeName}</span>
          )}
        </div>
      </div>
    );
  }, [getMergedCellInfo, isColumnSelected, level, renderTimeSlot, section.id, section.timeSlot]);

  const renderActionButtons = useCallback((section: Section) => (
    <div className="flex items-center justify-end gap-2">
      <button
        className={`p-1.5 rounded-lg transition-all duration-200 ${level >= MAX_SECTION_DEPTH ? "opacity-50 cursor-not-allowed bg-gray-100" : "hover:bg-gray-100"}`}
        onClick={() => onAddSubsection?.(section.id)}
        disabled={level >= MAX_SECTION_DEPTH}
        title={level >= MAX_SECTION_DEPTH ? "Maximum section depth reached" : "Add subsection"}
      >
  ...
  ), [activeTrack?.id, level, onAddSubsection, onUpdateSection, setFlyoverState]);

  const isColumnVisible = useCallback((type: string) => {
    const header = headers.find(h => h.type === type);
    return header?.isVisible ?? true;
  }, [headers]);

  const renderSubsections = useCallback((subsections: Section[], currentLevel: number) => {
    if (!subsections || subsections.length === 0) return null;

    return subsections.map((subsection: Section, index) => {
      // Create a unique key that includes all relevant data
      const subsectionKey = `${subsection.id}-${currentLevel}-${JSON.stringify(subsection.mergedFields)}-${subsection.subsections?.length}`;
      ...
      return (
        <React.Fragment key={subsectionKey}>
          ...
          {subsection.subsections && subsection.subsections.length > 0 && (
            renderSubsections(subsection.subsections, currentLevel + 1)
          )}
        </React.Fragment>
      );
    });
  ...
  return (
    <React.Fragment key={rowKey}>
      <tr className={`${getLevelColor(level, rowIndex)} ${level === 0 ? "font-medium text-gray-700" : "text-gray-600"}`}>
        {isColumnVisible('indicator') && (
          <td className="w-40 border-b border-r relative" style={{ borderColor: tableStyles.cellBorderColor, borderStyle: tableStyles.cellBorderStyle }}>
            <div className="absolute inset-0 flex items-center pl-2">{getLevelIndicator(level)}</div>
          </td>
        )}
        ...
        {isColumnVisible('actions') && !selection?.isSelecting && (
          <td className="px-4 py-2 border-b text-sm" style={{ borderColor: tableStyles.cellBorderColor, borderStyle: tableStyles.cellBorderStyle }}>
...
function SectionList({
  ...
  const [sectionFilter, setSectionFilter] = useState<{ type: "time" | "day" | "month"; value: { start?: string; end?: string; day?: string; month?: string } } | null>(null);
  ...
  const [filteredSections, setFilteredSections] = useState(groupSectionsByRole(sections));
  ...
  useEffect(() => {
    if (currentStyles) {
      setTableStyles({
        ...
        mainSectionGradientStart: currentStyles.mainSectionGradientStart || "#93c5fd80",
  ...
  const handleSelect = useCallback((cellIdOrSectionId: string, sectionId: string, columnType?: keyof MergedFields) => {
    ...
    setSelection((prev) => {
      const existingSelection = prev.selectedCells.find((cell) => cell.cellId === cellId);
      const existingMergedCell = prev.mergedCellsHistory.find((cell) => cell.sectionIds.includes(sectionId) && cell.columnType === actualColumnType);
      ...
      if (!existingSelection) {
        ...
        newSelectedCells = prev.selectedCells.filter((cell) => cell.cellId !== cellId);
      }
  ...
  const hasDifferentTypes = useCallback(() => {
    const types = new Set(selection.selectedCells.map(cell => cell.columnType));
    return types.size > 1;
  }, [selection.selectedCells]);

  const hasAlreadyMergedCells = useCallback(() => selection.selectedCells.some(cell => cell.mergeId), [selection.selectedCells]);

  const handleApplySelection = useCallback((color: string, mergeName: string) => {
    if (!mergeName.trim()) return alert("Please enter a merge name");
    if (selection.selectedCells.length < 1) return alert("Please select at least 1 cell to merge");
    ...
    executeMerge(color, mergeName);
  }, [hasAlreadyMergedCells, hasDifferentTypes, selection.selectedCells.length]);

  const handleUnmergeSelection = useCallback((mergeId: string, columnType: keyof MergedFields) => {
    const mergedCell = selection.mergedCellsHistory.find((cell) => cell.id === mergeId);
    if (!mergedCell) return;

    mergedCell.sectionIds.forEach((sectionId) => onUpdateSection(sectionId, {
      mergedFields: { [columnType]: { isMerged: false, color: "", mergeName: "", value: null, mergeId: "" } },
    }));

    setSelection((prev) => ({
      ...prev,
      mergedCellsHistory: prev.mergedCellsHistory.filter((cell) => cell.id !== mergeId),
      selectedCells: prev.selectedCells.filter((cell) => cell.mergeId !== mergeId),
    }));
  }, [onUpdateSection, selection.mergedCellsHistory]);

  const executeMerge = useCallback((color: string, mergeName: string) => {
    ...
    Object.entries(cellsByType).forEach(([columnType, cells]) => {
      ...
      const existingMergeGroups = Object.entries(cellsByMergeId).filter(([mergeId]) => mergeId !== 'unmerged').map(([_, cells]) => cells);
      const unmergedCells = cellsByMergeId['unmerged'] || [];
      const allSectionIds = new Set([...unmergedCells.map(cell => cell.sectionId), ...existingMergeGroups.flatMap(group => group.map(cell => cell.sectionId))]);

      const newMergeId = `merge-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      const newMergedCell: MergedCell = {
        ...
        value: cells[0].originalValue,
      };

      existingMergeGroups.forEach(group => {
        if (group[0].mergeId) handleUnmergeSelection(group[0].mergeId, columnType as keyof MergedFields);
      });

      Array.from(allSectionIds).forEach((sectionId) => onUpdateSection(sectionId, {
        mergedFields: { [columnType]: { isMerged: true, color, mergeName, value: cells[0].originalValue, mergeId: newMergeId } },
      }));

      setSelection((prev) => ({
        ...prev,
        mergedCellsHistory: [
          ...prev.mergedCellsHistory.filter(cell => cell.columnType !== columnType || !newMergedCell.sectionIds.some(id => cell.sectionIds.includes(id))),
          newMergedCell
        ],
      }));
    });
  ...
  }, [handleUnmergeSelection, onUpdateSection, selection.selectedCells]);

  const handleAddSubsection = useCallback((parentId: string) => {
    if (!activeTrack) return showToast.error("No active track selected");
    setFlyoverState({ isOpen: true, type: "add-subsection", data: { parentId, trackId: activeTrack.id } });
  }, [activeTrack, setFlyoverState, showToast]);

  const handleAddSection = useCallback(() => {
    if (!activeTrack) return showToast.error("No active track selected");
    setFlyoverState({ isOpen: true, type: "add-section", data: { trackId: activeTrack.id, parentId: null, sectionType: "program" } });
      ...
      filtered = filtered.map((group) => ({
        ...
        sections: group.sections.filter((section: Section) => {
          if (sectionFilter.type === "time" && sectionFilter.value.start) {
            return section.timeSlot.start === sectionFilter.value.start || section.timeSlot.end === sectionFilter.value.start;
          }
  ...
  const tableHeaders = useMemo(() => headers
    .filter((header) => header.isVisible && (!selection.isSelecting || header.type !== 'actions'))
    .map((header) => (
      <th
        key={header.id}
  ...
  const SelectionIndicator = useCallback(() => {
    ...
    const uniqueColumnTypes = new Set(selection.selectedCells.map(cell => cell.columnType));
    const uniqueMergeIds = new Set(selection.selectedCells.filter(cell => cell.mergeId).map(cell => cell.mergeId));
      ...
      <div className="flex-shrink-0">
        <div className="flex items-center gap-2 h-9">
          <span className="text-sm font-medium bg-gray-50/80 px-3 py-1.5 rounded-md whitespace-nowrap">
            {selection.selectedCells.length} cells selected ({Array.from(uniqueColumnTypes).join(", ")})
          </span>
          <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => {
                  uniqueMergeIds.forEach(mergeId => {
                    if (mergeId) {
                      const cell = selection.selectedCells.find(c => c.mergeId === mergeId);
                      if (cell) handleUnmergeSelection(mergeId, cell.columnType);
                    }
                  });
                }}
            ...
            <div className="flex items-center">
              ...
              >
                ...
                <span className="hidden md:inline">{hasMergedCells ? "New Merge" : "Merge"}</span>
              </button>
              <button
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-1 bg-white/70 backdrop-blur-sm border border-gray-100 rounded-xl p-1 shadow-sm transition-all duration-300 hover:shadow-md">
          ...
          >
            ...
            {selection.isSelecting && <span className="absolute -top-1 -right-1 w-2 h-2 bg-blue-500 rounded-full" />}
          </button>
          <button
            onClick={() =>
              setFlyoverState({
                ...
                data: {
                  ...
                  currentStyles: {
                    ...
                    mainSectionGradientStart: tableStyles.mainSectionGradientStart,
  ...
  }, [handleAddSection, headers, isFullScreen, selection.isSelecting, setFlyoverState, tableStyles, selection.selectedCells]);

  const MergeWarningModal = useCallback(() => (
    <Modal isOpen={showMergeWarning} onClose={() => setShowMergeWarning(false)} title="Warning: Different Column Types">
      <div className="p-4 space-y-4">
        <p className="text-gray-700">You are attempting to merge cells from different columns. This may cause unexpected behavior.</p>
        <div className="flex justify-end gap-3">
          <button onClick={() => setShowMergeWarning(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-md">Cancel</button>
          <button onClick={() => { setShowMergeWarning(false); executeMerge(pendingMergeColor, pendingMergeName); }} className="px-4 py-2 text-sm text-white bg-blue-600 hover:bg-blue-700 rounded-md">Proceed Anyway</button>
        </div>
      </div>
    </Modal>
  ), [executeMerge, pendingMergeColor, pendingMergeName, showMergeWarning]);

  const MergeConfirmationModal = useCallback(() => (
    <Modal isOpen={showMergeConfirmation} onClose={() => setShowMergeConfirmation(false)} title="Confirm Merge">
      <div className="p-4 space-y-4">
        <p className="text-gray-700">Some of the selected cells are already part of other merge groups. Proceeding will remove them from their current groups.</p>
        <div className="flex justify-end gap-3">
          <button onClick={() => setShowMergeConfirmation(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-md">Cancel</button>
          <button onClick={() => { setShowMergeConfirmation(false); executeMerge(pendingMergeColor, pendingMergeName); }} className="px-4 py-2 text-sm text-white bg-blue-600 hover:bg-blue-700 rounded-md">Proceed</button>
        </div>
      </div>
    </Modal>
  ), [executeMerge, pendingMergeColor, pendingMergeName, showMergeConfirmation]);

  return (
    <div className={`${isFullScreen ? "fixed inset-0 z-50 bg-white overflow-auto" : "relative flex flex-col h-full"}`}>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  <div className="flex items-center gap-3">
                    <h2 className="relative group">
                      <span className="text-xl font-bold bg-gradient-to-r from-blue-600 via-violet-500 to-purple-600 bg-clip-text text-transparent inline-flex items-center gap-2">
                        ...
                        <span className="font-semibold text-nowrap text-ellipsis overflow-hidden">{activeTrack.name}</span>
                      </span>
                  ...
                  <div className="flex w-full flex-col md:flex-row items-start md:items-center gap-3 md:gap-6">
                    <div className="flex min-w-40 md:w-auto items-center gap-2 bg-blue-50 px-3 py-1.5 rounded-lg">
                      ...
                      <span className="text-sm text-gray-600">{new Date(activeTrack.startDate).toLocaleDateString()} - {new Date(activeTrack.endDate).toLocaleDateString()}</span>
                    </div>
                    <div className="flex md:flex-row md:w-auto items-center gap-2 bg-violet-50 px-3 py-1.5 rounded-lg">
                      <Layers className="w-4 h-4 text-violet-500" />
                      <span className="text-sm text-gray-600 flex-nowrap min-w-20">{activeTrack.sections.length} sections</span>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                  ...
                  <div className="flex-1 max-w-md">
                    <div className="relative transform transition-all duration-200 hover:scale-[1.02]">
                      <SectionCalendarFilter sections={sections} onFilterChange={setSectionFilter} activeFilter={sectionFilter} />
                    </div>
                  </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody>
                        {displayedSections.map((group, groupIndex: number) =>
                          group.sections.map((section: Section, sectionIndex: number) => {
                            const sectionKey = `${section.id}-${JSON.stringify(section.mergedFields)}`;
                            return (
                              <React.Fragment key={sectionKey}>
                                <SectionRow
                                  ...
                                  tableStyles={tableStyles}
                                  rowIndex={groupIndex * group.sections.length + sectionIndex}
                                />
                              </React.Fragment>
                            );
                          })
      ...
      <ColorSelectionModal isOpen={showColorModal} onClose={() => setShowColorModal(false)} onApply={handleApplySelection} />
      <MergeWarningModal />
